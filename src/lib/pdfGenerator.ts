...
export class PDFInvoiceGenerator {
  ...
  constructor(platformSettings?: PlatformSettings) {
    this.doc = new jsPDF();
  ...
  private addHeader(): void {
    ...
    this.doc.text(this.platformSettings.platformName, this.margin, 25);
    
    // Tagline
    this.doc.setFont('helvetica', 'normal');
    this.doc.setFontSize(10);
    this.doc.text('The Future of Dairy, Today.', this.margin, 32);
  ...
  private addFooter(bill: Bill): void {
    ...
    this.doc.text(formatCurrency(bill.totalAmount), this.pageWidth - 115, footerY + 12);
    
    // Add PAID watermark for paid bills
    if (bill.isPaid) {
      this.doc.setFont('helvetica', 'bold');
      this.doc.setFontSize(60);
      this.doc.setTextColor(34, 139, 34, 0.1); // Semi-transparent green
      const text = 'PAID';
    ...
    this.doc.text(`Generated by ${this.platformSettings.platformName} - The Future of Dairy, Today.`, this.margin, this.pageHeight - 10);
  }
